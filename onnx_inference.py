...
class Predictor(object):
    ...
    def inference(self, ori_img, timer):
        img_info = {"id": 0}
        #ori_img = cv2.resize(ori_img, (800,800), interpolation = cv2.INTER_AREA)
        ...
        print(ort_inputs.items())
        timer.tic()
        output = self.session.run(None, ort_inputs)
        print('type output run session',type(output))
...
def imageflow_demo(predictor, args):
    ...
    while True:
        ...
        if ret_val:
            ...
            print("otuputs type is ",type(outputs))
